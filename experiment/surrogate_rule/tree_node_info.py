...
class tree_node_info:
    ...
    def train_surrogate_decision_tree(self):
        self.clf = DecisionTreeClassifier()
        self.clf.fit(self.cate_X, self.y_pred)
        if (self.verbose):
            print("***** finish training surrogate decision tree *****")
            print("surrogate overall fidelity:", self.clf.score(self.cate_X, self.y_pred))
    ...
    def tree_pruning(self, list = True):
    ...
    def derive_tree_data(self, node_id, parent_id, sign, dep):
        ...
        if (self.node_matched[node_id] > 0):
            ...
            #     node_info['fidelity'] = float(self.node_conf[node_id][pred//self.n_cls][pred%self.n_cls] / self.node_matched[node_id])
