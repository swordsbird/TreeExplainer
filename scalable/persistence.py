...
#################################################################################
def importTreeCollection(datasetName, silent=False, regressor=False):
    treeIndex = -1
    new_n_nodes = []
    new_children_left = []
    new_children_right = []
        ...
        for index in range(len(data)):
            ...
            if index <= 5:
                ...
                if not regressor:
                    ...
                    new_majorityClass[treeIndex].append(float(data[index][7]))
                if (data[index][1] == 'LN'):
...
def create_nodes(n_nodes, n_outputs, n_classes, children_left,
                 ...
                 impurities, n_samples, n_samples_norm, pruning, regressor=False):    
    def _create_node(cur, out_nodes, out_values, pruning):
        ...
        if l == r:
            ...
            if (pruning and dl==0 and dr==0 and (np.argmax(actual_values[cl]) == np.argmax(actual_values[cr])) and not regressor):
                dr = dl = -1
                out_nodes.pop()
                out_nodes.pop()
                out_values.pop()
    ...
    if not regressor:
        ...
        values = np.zeros((n_nodes, 1, n_outputs), dtype=np.float64)
...
def build_tree(X, y, n_features, n_classes, n_outputs, maxTreeDepth,
              n_nodes, children_left, children_right, features,
               thresholds, values, pruning=False, compute_score=False, regressor=False):
    ...
    if not regressor:
        ...
        actual_values = np.zeros((n_nodes, n_outputs, 1), dtype=np.float64)
        for i in range(n_nodes):
    ...
    d["nodes"], d["values"], d["max_depth"]  = create_nodes(n_nodes, n_outputs, n_classes,
                                                            ...
                                                            impurities, n_samples, n_samples_norm, pruning, regressor=regressor)
    ...
    if not regressor:
        ...
        tree_ = tree._tree.Tree(n_features, np.array([1] * n_outputs, dtype=np.intp), n_outputs)
...
def regressor_from_file(fn, X, y, pruning=False, compute_score=False, num_trees=-1):
    ...
    numOfClasses, maxTreeDepth, n_nodes, children_left, \
    children_right, features, thresholds, node_depths, \
    is_leaves, nodeValues, majorityClass = importTreeCollection(fn, silent=True, regressor=True)
    ...
    for i in range(numOfTrees):
        t = build_tree(X, y, n_features, np.array(n_classes, dtype=np.intp), n_outputs,
                       maxTreeDepth, n_nodes[i], children_left[i],
                       children_right[i], features[i], thresholds[i],
                       majorityClass[i], pruning, compute_score, regressor=True)
        trees.append(t)
